...
const BOOKING_STATUS_RU = { pending: 'Ожидает', booked: 'Оплачена', active: 'В аренде', completed: 'Завершена', cancelled: 'Отменена' };
...
const STAGES = [
  ...
  { key: 'await_payment', label: 'Ожидает оплаты бронирования', icon: 'ph-hourglass' },
  { key: 'paid', label: 'Бронирование оплачено', icon: 'ph-link' },
...
const statusTagClass = (st) => st === 'completed' ? 'done' : st === 'cancelled' ? 'cancel' : (st === 'active' || st === 'booked') ? 'ok' : '';
...
export default function Admin() {
  ...
  const handleBlogImage = async (e) => {
    ...
    if (dataUrlBytes(data) > 8 * 1024 * 1024) { alert('Файл слишком большой даже после сжатия.'); return; }
    setBlogForm(f => ({ ...f, image_url: data }));
  };
  ...
  // Фото сжимаются на клиенте (айфон-фото 4–8 МБ иначе не влезут в лимит).
  const uploadPhotoFile = async (carId, e) => {
    ...
    for (const file of files) {
      try {
        ...
        if (dataUrlBytes(dataUrl) > 8 * 1024 * 1024) { alert(`Файл «${file.name}» слишком большой даже после сжатия.`); continue; }
